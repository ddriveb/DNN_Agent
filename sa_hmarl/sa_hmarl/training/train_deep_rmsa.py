"""DeepRMSA A3C training script for SA-HMARL Agent-R slot.

Replaces Agent-R DQN with DeepRMSA's A3C algorithm, keeping the same
environment, observations, and evaluation protocol.

Run (from project root):
    PYTHONPATH=sa_hmarl .venv/bin/python -m sa_hmarl.training.train_deep_rmsa \
...
def evaluate_agent(env: SMDPEnv,
    ...
    """Evaluate DeepRMSA agent (greedy) on a fixed request sequence."""
    env.reset(requests)
    agent.eval()  # argmax mode
    total_reward = 0.0
    blocked = 0
    total_waste = 0.0
    num_success = 0
...
def train(args):
    """Run DeepRMSA A3C training loop."""
    env = make_env(
    ...
    print(f"LR: {args.lr}, entropy: {args.entropy_coef}")
    print(f"k_path: {env.k}, m_blocks: {args.m_blocks}, n_actions: {agent.n_actions}")
    print("=" * 70)
    ...
    for episode in range(args.episodes):
        ...
        for t, req in enumerate(requests):
            ...
            is_terminal = (t + 1 >= len(requests))
            agent.store_transition(reward, is_terminal)

        # Episode-level A2C update (terminal episode → bootstrap_value=0.0)
        loss_dict = agent.optimize(bootstrap_value=0.0)
        if loss_dict is not None:
            metrics["losses"].append(loss_dict["loss"])
            metrics["policy_losses"].append(loss_dict["policy_loss"])
            metrics["value_losses"].append(loss_dict["value_loss"])
            metrics["entropies"].append(loss_dict["entropy"])
        ...
        if episode % 100 == 0 or episode == args.episodes - 1:
            ...
            loss_str = ""
            if loss_dict is not None:
                loss_str = f"| loss={loss_dict['loss']:.3f} pol={loss_dict['policy_loss']:.3f} val={loss_dict['value_loss']:.3f} ent={loss_dict['entropy']:.3f}"
    ...
    print(f"  DeepRMSA: reward={final_eval['avg_reward']:+.3f}, "
    ...
    if args.checkpoint_name:
        ...
        ckpt_name = "deep_rmsa_mixed.pt" if args.mixed_splits else "deep_rmsa_smoke.pt"
...
def main():
    parser = argparse.ArgumentParser(description="DeepRMSA A3C training")
    ...
    # DeepRMSA-specific hyperparameters
    parser.add_argument("--lr", type=float, default=1e-5,
                        help="Learning rate (DeepRMSA original: 1e-5)")
    ...
    parser.add_argument("--m_blocks", type=int, default=1,
                        help="Number of FS starting positions to consider per path (DeepRMSA default: 1)")
    parser.add_argument("--k_paths", type=int, default=3,
                        help="Number of candidate paths exposed to DeepRMSA")
    parser.add_argument("--checkpoint_name", type=str, default=None,
                        help="Optional checkpoint filename under sa_hmarl/checkpoints")
    parser.add_argument("--batch_size", type=int, default=32,
                        help="Deprecated: kept for backward compatibility, ignored in episode-level A2C")
