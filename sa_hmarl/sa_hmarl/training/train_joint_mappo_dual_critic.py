"""Heterogeneous Dual-Critic MAPPO training for Agent-C and Agent-R.

This extends the single centralized-critic MAPPO into a true dual-critic
setup where:
  - Critic-C estimates V_C(s, a_C) and drives Agent-C updates.
  - Critic-R estimates V_R(s, a_R) and drives Agent-R updates.

Both critics are centralized (see global summary + the other agent's
action-space mean), but each is trained on its own reward stream and
produces its own advantage for its respective actor.

Key differences from single-critic MAPPO:
  1. Two independent value networks (V_C and V_R).
  2. Two GAE passes (one per reward stream).
  3. Each actor updated with its own advantage.
  4. Four checkpoints saved (c_actor, r_actor, critic_c, critic_r).
"""
...
# ------------------------------------------------------------------
# Dual-Critic Transition and Rollout Buffer
# ------------------------------------------------------------------

@dataclass
class DualPPOJointTransition:
    """One joint step with dual-critic values and decomposed rewards."""
    ...
    r_log_prob: float
    r_valid: bool
    value_features_c: np.ndarray
    value_c: float
    value_features_r: np.ndarray
    value_r: float
    reward_c: float
    reward_r: float
    done: bool
...
class DualPPORolloutBuffer:
    """Episode rollout storage with per-agent GAE support."""

    def __init__(self):
        self.transitions: List[DualPPOJointTransition] = []

    def add(self, transition: DualPPOJointTransition):
        self.transitions.append(transition)

    def __len__(self) -> int:
        return len(self.transitions)

    def _compute_gae(self, rewards, values, dones, gamma, gae_lambda, normalize):
    ...
    def compute_gae_c(self, gamma: float, gae_lambda: float, normalize: bool = True):
        rewards = np.array([t.reward_c for t in self.transitions], dtype=np.float32)
        values = np.array([t.value_c for t in self.transitions], dtype=np.float32)
        dones = np.array([t.done for t in self.transitions], dtype=np.float32)
        return self._compute_gae(rewards, values, dones, gamma, gae_lambda, normalize)

    def compute_gae_r(self, gamma: float, gae_lambda: float, normalize: bool = True):
        rewards = np.array([t.reward_r for t in self.transitions], dtype=np.float32)
        values = np.array([t.value_r for t in self.transitions], dtype=np.float32)
        dones = np.array([t.done for t in self.transitions], dtype=np.float32)
        return self._compute_gae(rewards, values, dones, gamma, gae_lambda, normalize)


# ------------------------------------------------------------------
# Dual Centralized Value Critics
# ------------------------------------------------------------------

class CentralizedValueNetwork(nn.Module):
    """MLP value function V(s, a)."""
...
class DualCentralizedValueCritic:
    """Two centralized critics: V_C for Agent-C, V_R for Agent-R.

    Critic-C input:  c_mean || c_selected || r_mean || global_summary
    Critic-R input:  c_mean || r_mean || r_selected || global_summary
    """
    ...
    def __init__(
        ...
        self.input_dim_c = c_dim + c_dim + r_dim + global_dim
        self.input_dim_r = c_dim + c_dim + r_dim + r_dim + global_dim
        self.hidden_dims = tuple(hidden_dims)
        self.device = device

        self.value_net_c = CentralizedValueNetwork(self.input_dim_c, hidden_dims).to(device)
        self.value_net_r = CentralizedValueNetwork(self.input_dim_r, hidden_dims).to(device)
        self.optimizer_c = torch.optim.Adam(self.value_net_c.parameters(), lr=lr)
        self.optimizer_r = torch.optim.Adam(self.value_net_r.parameters(), lr=lr)
    ...
    def encode_c(
        ...
        return np.concatenate([c_mean, c_sel, r_mean, global_summary]).astype(np.float32)
    ...
    def encode_r(
        ...
        if env is None:
            ...
            global_summary = self._global_summary(env, c_mask, r_mask)
        return np.concatenate([c_mean, c_sel, r_mean, r_sel, global_summary]).astype(np.float32)

    def value_c(self, value_features: np.ndarray) -> float:
        x = torch.tensor(value_features, dtype=torch.float32, device=self.device).unsqueeze(0)
        with torch.no_grad():
            return float(self.value_net_c(x).item())

    def value_r(self, value_features: np.ndarray) -> float:
        x = torch.tensor(value_features, dtype=torch.float32, device=self.device).unsqueeze(0)
        with torch.no_grad():
            return float(self.value_net_r(x).item())
    ...
    def optimize_c(self, value_features, returns, old_values=None, epochs=4, clip_coef=None):
        return self.optimize(self.value_net_c, self.optimizer_c, value_features, returns, old_values, epochs, clip_coef)

    def optimize_r(self, value_features, returns, old_values=None, epochs=4, clip_coef=None):
        return self.optimize(self.value_net_r, self.optimizer_r, value_features, returns, old_values, epochs, clip_coef)


# ------------------------------------------------------------------
# Helpers (copied from single-critic MAPPO)
# ------------------------------------------------------------------
...
def _entropy_coef_for_episode(args, episode: int) -> float:
    ...
    return float(args.entropy_coef + progress * (args.entropy_final_coef - args.entropy_coef))
...
def _load_reference_ppo_policy(ckpt_path: str, device: str) -> MaskedPPOActorNetwork:
...
# ------------------------------------------------------------------
# Evaluation (same as single-critic MAPPO)
# ------------------------------------------------------------------
...
def _evaluate_on_val_set(env, agent_c, agent_r, episodes_list, args=None):
    ...
    return {
        ...
        "avg_delay_ms": total_delay / max(total_success, 1),
        "avg_fs": total_fs / max(total_success, 1),
...
def _save_checkpoints(ckpt_dir, prefix, suffix, agent_c, agent_r, critic, args, metrics):
    ...
    torch.save(
        {
            "model_state": critic.value_net_c.state_dict(),
            "input_dim": critic.input_dim_c,
            "hidden_dims": critic.hidden_dims,
            "args": vars(args),
            "training_metrics": metrics,
        },
        str(ckpt_dir / f"{prefix}_critic_c_{suffix}.pt"),
    )
    torch.save(
        {
            "model_state": critic.value_net_r.state_dict(),
            "input_dim": critic.input_dim_r,
            "hidden_dims": critic.hidden_dims,
            "args": vars(args),
            "training_metrics": metrics,
        },
        str(ckpt_dir / f"{prefix}_critic_r_{suffix}.pt"),
    )
...
def train(args):
    ...
    max_mod_se = max(m.spectral_efficiency for m in mod_reg.mod_table) if mod_reg.mod_table else 1.0
    agent_c = PPOAgentC(
        input_dim=17,
        hidden_dims=(128, 64),
        lr=args.lr_actor,
    ...
    critic = DualCentralizedValueCritic(
    ...
    if args.r_ref_kl_coef > 0 and args.reference_r_checkpoint:
        ref_r_policy = _load_reference_ppo_policy(args.reference_r_checkpoint, args.device)
    ...
    print("=" * 72)
    print("Dual-Critic MAPPO Joint Training")
    ...
    if args.r_se_bonus_coef > 0 or args.r_fs_usage_coef > 0 or args.r_low_se_penalty > 0:
        print(
            f"Agent-R SE shaping: se_bonus={args.r_se_bonus_coef}, "
            f"fs_usage={args.r_fs_usage_coef}, low_se_penalty={args.r_low_se_penalty}"
        )
    ...
    if args.r_ref_kl_coef > 0:
        print(f"R ref KL coef: {args.r_ref_kl_coef}  lr_r_actor: {lr_r}")
    if ref_r_policy is not None:
        print(f"R KL reference: {args.reference_r_checkpoint}")
    print(f"Critic-C input dim: {critic.input_dim_c}")
    print(f"Critic-R input dim: {critic.input_dim_r}")
    ...
    for episode in range(args.episodes):
        ...
        for t, req in enumerate(requests):
            ...
            action_idx_c, logp_c, _ = agent_c.select_from_features(c_features, c_mask, deterministic=False)
            ...
            action_idx_r, logp_r, _ = agent_r.select_from_features(r_features, r_mask, deterministic=False)
            ...
            value_features_c = critic.encode_c(c_features, c_mask, action_idx_c if c_valid else 0,
                                                r_features, r_mask, env=env)
            value_c = critic.value_c(value_features_c)
            value_features_r = critic.encode_r(c_features, c_mask,
                                                action_idx_c if c_valid else 0,
                                                r_features, r_mask,
                                                action_idx_r if r_valid else 0, env=env)
            value_r = critic.value_r(value_features_r)
            ...
            reward_r = compute_reward(
                info, args.waste_coef, mod_name=mod_name,
            ...
            if (args.r_bc_coef > 0 and ref_agent_r is not None
                ...
                ref_mod = _get_reference_mod_name(ref_agent_r, obs_r, obs_r["mod_names"])
                if ref_mod is not None and ref_mod in ("BPSK", "QPSK", "8QAM"):
                    reward_r -= args.r_bc_coef

            # R-side SE-aware reward shaping (success only)
            if info.get("success", False):
                if args.r_se_bonus_coef > 0 and mod_name is not None:
                    mod_fmt = mod_reg.by_name(mod_name)
                    if mod_fmt is not None and max_mod_se > 0:
                        reward_r += args.r_se_bonus_coef * (mod_fmt.spectral_efficiency / max_mod_se)
                if args.r_fs_usage_coef > 0:
                    reward_r -= args.r_fs_usage_coef * (info.get("num_slots", 0) / max(env.net.num_slots, 1))
                if args.r_low_se_penalty > 0 and mod_name in ("BPSK", "QPSK"):
                    reward_r -= args.r_low_se_penalty

            if not info.get("success", False):
                blocked += 1
                reasons[info.get("reason", "unknown")] += 1

            rollout.add(DualPPOJointTransition(
                c_features=c_features, c_mask=c_mask,
                c_action=action_idx_c if action_idx_c is not None else 0,
                c_log_prob=logp_c, c_valid=c_valid,
                r_features=r_features, r_mask=r_mask,
                r_action=action_idx_r if action_idx_r is not None else 0,
                r_log_prob=logp_r, r_valid=r_valid,
                value_features_c=value_features_c, value_c=value_c,
                value_features_r=value_features_r, value_r=value_r,
                reward_c=reward_c, reward_r=reward_r,
                done=(t == len(requests) - 1), info=info,
            ))

        transitions = rollout.transitions
        advantages_c, returns_c, old_values_c = rollout.compute_gae_c(
            args.gamma, args.gae_lambda, normalize=not args.no_advantage_norm
        )
        advantages_r, returns_r, old_values_r = rollout.compute_gae_r(
            args.gamma, args.gae_lambda, normalize=not args.no_advantage_norm
        )

        value_loss_c = critic.optimize_c(
            [t.value_features_c for t in transitions], returns_c,
            old_values=old_values_c, epochs=args.ppo_epochs,
            clip_coef=args.value_clip_coef,
        )
        value_loss_r = critic.optimize_r(
            [t.value_features_r for t in transitions], returns_r,
            old_values=old_values_r, epochs=args.ppo_epochs,
            clip_coef=args.value_clip_coef,
        )
        metrics["losses_critic_c"].append(value_loss_c)
        metrics["losses_critic_r"].append(value_loss_r)
        ...
        if c_indices:
            c_loss, _, c_kl, _ = agent_c.optimize_ppo(
                ...
                advantages_c[c_indices], args.clip_coef,
                epochs=args.ppo_epochs, target_kl=args.target_kl,
            )
            metrics["losses_c"].append(c_loss)
            metrics["kl_c"].append(c_kl)

        r_features, r_masks, r_actions, r_old_logp, r_indices = _filter_actor_batch(transitions, "r")
        ...
        if r_frozen:
            ...
            r_loss, _, r_kl, r_ref_kl = agent_r.optimize_ppo(
                r_features, r_masks, r_actions, r_old_logp,
                advantages_r[r_indices], args.clip_coef,
                epochs=args.ppo_epochs, target_kl=args.target_kl,
                reference_policy=ref_r_policy, ref_kl_coef=args.r_ref_kl_coef,
            )
        else:
            r_loss = 0.0
            r_kl = 0.0
        ...
        if (episode > 0 and episode % args.eval_freq == 0) or episode == args.episodes - 1:
            ...
            avg_delay = float(np.mean([r["avg_delay_ms"] for r in eval_results.values()]))
            avg_fs = float(np.mean([r["avg_fs"] for r in eval_results.values()]))
            ...
            metrics["eval_delay_ms"].append(avg_delay)
            metrics["eval_fs"].append(avg_fs)
            ...
            if avg_blocking < best_blocking:
                ...
                print(
                    f"  >> Best Dual-Critic MAPPO @ ep {episode}: avg_blk={avg_blocking:.3f} "
                    f"avg_r={avg_reward:+.3f} avg_delay={avg_delay:.1f}ms avg_fs={avg_fs:.2f} "
            ...
            if args.print_eval_diagnostics:
                ...
                reason_str = ", ".join(f"{k}={v}" for k, v in all_reasons.most_common(6)) or "none"
                mod_total = max(sum(all_mods.values()), 1)
                mod_str = ", ".join(f"{k}={v / mod_total:.1%}" for k, v in all_mods.most_common()) or "none"
        ...
        if episode % args.log_interval == 0 or episode == args.episodes - 1:
            ...
            ref_str = f" ref_kl={r_ref_kl:.4f}" if args.r_ref_kl_coef > 0 else ""
            print(
                ...
                f"vl_c={value_loss_c:.4f} vl_r={value_loss_r:.4f}"
            )
            if reason_str:
                print(f"         Reasons: {reason_str}")

    _save_checkpoints(ckpt_dir, args.ckpt_prefix, "last", agent_c, agent_r, critic, args, metrics)
    print(f"Best Dual-Critic MAPPO checkpoint: avg_blk={best_blocking:.3f} @ ep {best_episode}")
    return agent_c, agent_r, critic, metrics


def main():
    parser = argparse.ArgumentParser(description="Dual-Critic MAPPO joint PPO training")
    ...
    parser.add_argument("--r_se_bonus_coef", type=float, default=0.0)
    parser.add_argument("--r_fs_usage_coef", type=float, default=0.0)
    parser.add_argument("--r_low_se_penalty", type=float, default=0.0)
    ...
    parser.add_argument("--ckpt_prefix", type=str, default="joint_mappo_dual_critic")
