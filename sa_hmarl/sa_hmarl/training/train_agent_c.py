...
def train(args):
    ...
    if args.feature_mode in ("mean_field", "typed_mean_field", "gated_typed_mean_field"):
        agent_c_kwargs["num_servers"] = args.num_servers
    agent = AgentC(**agent_c_kwargs)
    ...
    ckpt_best = ckpt_dir / "agent_c_best.pt"
    ckpt_last = ckpt_dir / "agent_c_last.pt"
    ...
    print("=" * 70)
    print("Agent-C DQN Training (fixed KSP-BF RMSA)")
    print(f"Feature mode: {args.feature_mode}, Input dim: {agent.input_dim}")
...
def main():
    parser = argparse.ArgumentParser(description="Agent-C DQN training")
    ...
    parser.add_argument("--feature_mode", type=str, default="default",
                        choices=["default", "enhanced", "pressure_aware",
                                 "cross_pressure", "r_feasibility",
                                 "mean_field", "typed_mean_field",
                                 "gated_typed_mean_field"],
                        help="Agent-C feature mode")
