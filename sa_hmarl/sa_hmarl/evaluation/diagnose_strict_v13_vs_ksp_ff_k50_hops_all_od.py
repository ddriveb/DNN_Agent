#!/usr/bin/env python3
"""Diagnosis: Strict v1.3 vs KSP-FF K=50 hops under fixed-C / all-OD.
...
def _load_deep_rmsa(ckpt_path: str, env, mod_reg: ModulationRegistry, device: str = "cpu"):
    ...
    if ckpt_nodes is not None and ckpt_nodes != env.net.NUM_NODES:
        raise ValueError(
            f"DeepRMSA checkpoint topology mismatch: ckpt nodes={ckpt_nodes}, env nodes={env.net.NUM_NODES}"
        )
    ckpt_slots = ckpt.get("num_slots")
    if ckpt_slots is not None and ckpt_slots != env.net.num_slots:
        raise ValueError(
            f"DeepRMSA checkpoint slot mismatch: ckpt slots={ckpt_slots}, env slots={env.net.num_slots}"
        )
    agent = DeepRMSAAgent(
        num_nodes=env.net.NUM_NODES,
        num_slots=env.net.num_slots,
        k_path=ckpt.get("k_path", 3),
        m_blocks=ckpt.get("m_blocks", 1),
        mod_registry=mod_reg,
        device=device,
    )
    agent.policy_net.load_state_dict(ckpt["policy_net"])
    agent.value_net.load_state_dict(ckpt["value_net"])
    agent.eval()
...
def _select_r_action_deep_rmsa(deep_rmsa, obs_r: Dict[str, Any]) -> Tuple[Optional[int], bool, Dict[str, Any]]:
    """Return (r_idx, r_valid, info) for DeepRMSA."""
    info: Dict[str, Any] = {}
    a = deep_rmsa.select_action(obs_r)
    if a is None:
        return None, False, info
    return int(a), True, info
...
def _run_episode(
    ...
    mode: str,  # "strict" or "ksp_ff" or "deep_rmsa"
    args: argparse.Namespace,
) -> Tuple[MethodSummary, List[FixedRequestRecord]]:
    """Run one episode under fixed-C / all-OD."""
    assert mode in ("strict", "ksp_ff", "deep_rmsa")
    records: List[FixedRequestRecord] = []
    summary = MethodSummary(
        method=STRICT_V13_NAME if mode == "strict" else ("DeepRMSA" if mode == "deep_rmsa" else KSP_FF_NAME)
    )
    num_mods = env.mod_reg.num_formats
    max_blocks = env.max_blocks

    # Map dst_node -> server_id (one server per node).
    ...
    for step_idx, req in enumerate(requests):
        ...
        if server_id is None:
            # Fallback: choose the server with closest node_id.
            server_id = min(node_to_server.values(), key=lambda i: abs(i - dst_node))

        # R-side observation under fixed split_id and deterministic server_id.
        ...
        if mode == "strict":
            ...
            r_idx, r_valid, r_info = _select_r_action_deep_rmsa(args._deep_rmsa, obs_r)
...
def _diagnose(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    # Load DeepRMSA if requested.
    deep_rmsa = None
    if args.r_method == "deep_rmsa":
        env_for_deep = make_env(
            args.topology,
            args.num_slots,
            args.num_servers,
            42,
        ...
        deep_rmsa = _load_deep_rmsa(args.deep_rmsa_checkpoint, env_for_deep, mod_reg, args.device)
        args._deep_rmsa = deep_rmsa
    ...
    for seed in seeds:
        ...
        env_first.reset(requests)
        first_mode = args.r_method
        first_summary, first_records = _run_episode(
            env_first, requests, agent_r, ranker, first_mode, args
        )
        ...
        paired = _build_paired_records(first_records, ksp_records)
        all_paired.extend(paired)
        all_strict_records.extend(first_records)
        all_ksp_records.extend(ksp_records)

        per_seed_summaries.append({
            "seed": seed,
            "strict": first_summary.to_dict(),
            "ksp_ff": ksp_summary.to_dict(),
            "strict_minus_ksp_blocking_pp": (first_summary.blocking_rate() - ksp_summary.blocking_rate()) * 100.0,
        })

        print(
            f"[diagnose][seed={seed}] "
            f"{first_summary.method} blocking={first_summary.blocking_rate():.4%} "
            f"KSP-FF blocking={ksp_summary.blocking_rate():.4%} "
            f"Δ={(first_summary.blocking_rate() - ksp_summary.blocking_rate()) * 100:+.2f}pp"
        )
...
def _build_markdown(payload: Dict[str, Any]) -> str:
    ...
    lines = [
        f"# Diagnosis: {STRICT_V13_NAME} vs {KSP_FF_NAME} (fixed-C / all-OD)",
        "",
        "## Protocol Lock",
        "",
        ...
        f"- server_node_ids: {od['server_node_ids']}",
...
def _build_next_step_decision(payload: Dict[str, Any]) -> str:
    ...
    if blocking_diff_pp < -0.5:
        ...
        lines.append("")
        lines.append("Recommended follow-ups:")
        lines.append("- Try a different traffic matrix or OD distribution with higher spectral contention.")
    ...
    lines.append(f"- traffic_matrix: {od['traffic_matrix']}")
    lines.append(f"- fixed_split_id: {od['fixed_split_id']}")
    lines.append(f"- server_node_ids: {od['server_node_ids']}")
    lines.append("")
    return "\n".join(lines)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--deep_rmsa_checkpoint",
        default="sa_hmarl/checkpoints/deep_rmsa_snap24_reach_mixed.pt",
    )
    parser.add_argument(
        "--r_method",
        choices=["strict", "ksp_ff", "deep_rmsa"],
        default="strict",
        help="R-side method to compare against KSP-FF K=50 hops.",
    )
    ...
    parser.add_argument(
        "--output_dir",
        default="sa_hmarl/experiments/strict_v13_vs_ksp_ff_k50_hops_all_od_diagnosis",
    )
