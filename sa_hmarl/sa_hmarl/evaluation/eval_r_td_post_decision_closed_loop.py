"""Closed-loop evaluation of R-side TD post-decision value reranking."""
...
def _select_post_r_action(
    ...
    if mode == "td_post_only":
        scores = values
    elif mode == "ppo_td_post_rerank":
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if "ppo_td_post_rerank" in requested:
        for value in [float(v) for v in args.lambda_values.split(",") if v.strip()]:
            methods[f"ppo_td_post_rerank_{value:g}"] = {"per_seed": {}, "lambda": value}
    if "td_post_only" in requested:
        methods["td_post_only"] = {"per_seed": {}}
        ...
        for seed in seeds:
            ...
            for requests in episodes_by_seed[seed]:
                ...
                if method_name == "ppo_r":
                    mode = "ppo_r"
                elif method_name == "td_post_only":
                    mode = "td_post_only"
                elif method_name == "post_only":
                    mode = "post_only"
                elif method_name == "deep_rmsa":
                    mode = "deep_rmsa"
                elif method_name.startswith("ppo_td_post_rerank"):
                    mode = "ppo_td_post_rerank"
                else:
    ...
    return {
        "config": vars(args),
        "post_checkpoint_mode": checkpoint.get("mode", "td"),
        "methods": methods,
    }
...
def _write_markdown(path: Path, report: Dict[str, Any]) -> None:
    lines = [
        "# R-Side TD Post-Decision Closed-Loop Evaluation", "",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--methods", default="ppo_r,td_post_only,ppo_td_post_rerank,deep_rmsa")
    ...
    parser.add_argument("--output_json", default="sa_hmarl/experiments/r_td_post_decision_closed_loop.json")
    parser.add_argument("--output_md", default="sa_hmarl/experiments/r_td_post_decision_closed_loop.md")
    return parser
