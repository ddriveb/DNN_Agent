"""Diagnostic: why R-post loses to DeepRMSA on K5/M10.

Runs three sets of analyses on the same request traces used for the formal
system comparison:

1. Separate trajectories for r_post / deep_rmsa / ppo_r, recording per-request
   legal R action count, selected_valid_r_actions, selected action features,
   and outcome (blocked / reason).
2. A probe pass over the DeepRMSA trajectory: at every request, after the C
   decision, query what r_post and ppo_r would do in the same state, compute
   the full post-value distribution over legal R actions, and record ranks.
3. Aggregate all of the above into a JSON + markdown diagnostic report.
"""
...
def _run_probe_on_deep_rmsa_trajectory(
    episodes_by_seed: Dict[int, List[List[Any]]],
    agent_c,
    agent_r,
    deep_rmsa,
    ...
    """Run DeepRMSA trajectory and probe what r_post / ppo_r would do at each state."""
    model, mean, std = post_pack
    probe_records = []
    per_seed_metrics = {}
    for seed, episodes in episodes_by_seed.items():
        ...
        for requests in episodes:
            ...
            for req in requests:
                ...
                post_idx = _select_post_r_action(
                    env, req, obs_c, obs_r, agent_r, model, mean, std,
                    split_id, server_id, "post_only", 1.0, args.device,
                )
                ...
                # Step with DeepRMSA action.
                r_action = (0, 0, 0) if deep_idx is None else decode_agent_r_action(
                ...
                probe_records.append({
                    ...
                    "r_post_idx": int(post_idx),
                    "ppo_r_idx": int(ppo_idx),
                    "deep_rmsa_rank_in_post_value": val_dist["ranks"].get(int(deep_idx) if deep_idx is not None else -1),
                    "ppo_r_rank_in_post_value": val_dist["ranks"].get(int(ppo_idx)),
                    "r_post_rank_in_post_value": val_dist["ranks"].get(int(post_idx)),
                    ...
                    "post_value_of_r_post": val_dist["values"].get(int(post_idx)),
                    ...
                    "deep_rmsa_success": bool(info.get("success", False)),
                })
        per_seed_metrics[str(seed)] = _aggregate_metrics(metrics)
        print(f"[probe on deep_rmsa] completed seed {seed}", flush=True)
    aggregate = {
        key: float(np.mean([row[key] for row in per_seed_metrics.values()]))
        for key in next(iter(per_seed_metrics.values()))
        if isinstance(next(iter(per_seed_metrics.values()))[key], (int, float))
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    methods = ["r_post", "deep_rmsa", "ppo_r"]
    all_records = []
    separate_metrics = {}
    for method in methods:
    ...
    probe_records, probe_metrics = _run_probe_on_deep_rmsa_trajectory(
    ...
    agreement_post_deep, n_post_deep = _action_agreement(all_records, "r_post", "deep_rmsa")
    agreement_post_ppo, n_post_ppo = _action_agreement(all_records, "r_post", "ppo_r")
    agreement_deep_ppo, n_deep_ppo = _action_agreement(all_records, "deep_rmsa", "ppo_r")

    # Probe summaries.
    deep_ranks = [r["deep_rmsa_rank_in_post_value"] for r in probe_records if r["deep_rmsa_rank_in_post_value"] is not None]
    ppo_ranks = [r["ppo_r_rank_in_post_value"] for r in probe_records if r["ppo_r_rank_in_post_value"] is not None]
    post_values = [r["post_value_of_r_post"] for r in probe_records if r["post_value_of_r_post"] is not None]
    ...
    report = {
        ...
        "agreement": {
            "r_post_vs_deep_rmsa": {"rate": agreement_post_deep, "pairs": n_post_deep},
            "r_post_vs_ppo_r": {"rate": agreement_post_ppo, "pairs": n_post_ppo},
        ...
        "probe_summary": {
            ...
            "r_post_selected_value_mean": float(np.mean(post_values)) if post_values else None,
            "r_post_selected_value_std": float(np.std(post_values)) if post_values else None,
...
def _write_markdown(path: Path, report: Dict[str, Any]) -> None:
    lines = ["# R-Post vs DeepRMSA Failure Diagnostic", ""]
    ...
    lines += ["## 3. DeepRMSA action rank under R-post value (on DeepRMSA states)", ""]
    ...
    lines += [
        f"- R-post selected value mean ± std: {ps['r_post_selected_value_mean']:.4f} ± {ps['r_post_selected_value_std']:.4f}",
    ...
    lines += [
        "1. If R-post selected value mean is close to or lower than DeepRMSA selected value mean,",
        "   the value network is not confidently picking better actions than DeepRMSA.",
        "2. If DeepRMSA's rank under R-post value is usually > 1, R-post disagrees with DeepRMSA",
        "   on states where DeepRMSA succeeds; the disagreement is costly.",
        "3. Compare feature means: if R-post picks systematically longer paths, lower modulation,",
        "   or larger block waste, the value model has learned a spurious inductive bias.",
        "4. Minimal next steps:",
        "   - Increase `label_future_block_coef` / `label_future_nsb_coef` further, or extend H.",
        "   - Add explicit future-NSB-aware features (e.g., expected block consumption).",
        "   - Distill from DeepRMSA as a teacher (clone/improve its choices).",
        "   - Use a larger hidden network and stronger ranking regularization.",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--post_checkpoint", default="sa_hmarl/checkpoints/r_post_decision_h5_k5m10_blocking_focused/c_post_decision_joint.pt")
    ...
    parser.add_argument("--output_json", default="sa_hmarl/experiments/r_post_h5_k5m10_failure_diagnostic.json")
    parser.add_argument("--output_md", default="sa_hmarl/experiments/r_post_h5_k5m10_failure_diagnostic.md")
    return parser
