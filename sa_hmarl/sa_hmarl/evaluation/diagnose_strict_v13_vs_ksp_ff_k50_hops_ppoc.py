    ...
    for step_idx, req in enumerate(requests):
        ...
        if not c_valid:
            ...
            if not is_warmup:
                ...
                rec.decision_ms = (time.perf_counter() - t0) * 1000.0
                records.append(rec)
            ...
            if idx is not None:
                for j in range(idx - 1, -1, -1):
                    ...
                    if prev.seed != item.seed:
                        continue
                    if not getattr(prev, divergence_key):
...
def _mechanism_summary(payload: Dict[str, Any]) -> str:
    ...
    if strict_failures:
        ...
        if top_reason[0] == "server_overload" or top_reason[0] == "server_saturated":
            paragraphs.append(
                ...
                "into states where later requests encounter server overload."
            )
        elif top_reason[0] == "no_suitable_block":
            paragraphs.append(
                f"Among strict_win requests, the dominant KSP-FF K=50 hops failure reason is "
                f"'{top_reason[0]}' ({top_reason[1]} occurrences). "
                "This indicates that KSP-FF's greedy choices fragment the spectrum, leaving later "
                "requests without a suitable optical block."
            )
        elif top_reason[0] in ("c_no_valid_action", "r_no_valid_action"):
            paragraphs.append(
                f"Among strict_win requests, the dominant KSP-FF K=50 hops failure reason is "
                f"'{top_reason[0]}' ({top_reason[1]} occurrences). "
                "This indicates that KSP-FF's trajectory leaves the PPO-C agent with no valid "
                "(split, server) action, likely because server and/or spectrum resources are exhausted."
            )
        else:
            paragraphs.append(
                f"Among strict_win requests, the dominant KSP-FF K=50 hops failure reason is "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--fixed_c", action="store_true")
