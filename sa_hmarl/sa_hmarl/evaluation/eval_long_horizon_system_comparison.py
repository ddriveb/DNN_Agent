"""Long-horizon system-level comparison for imported benchmark topologies.

This script is a variant of eval_main_s100_system_comparison tailored for
fair, high-load benchmarking across imported topologies (COST239, German17,
NSFNET, JPN48).  It supports:

- Long episodes (e.g. 10 000 requests per seed)
- Warmup / measurement split
- Optional Poisson arrivals and exponential holding times
- Per-request random source nodes (destination is determined by the selected
  edge server, which is the natural compute-optical project behavior)

It evaluates R-side backends under the same learned PPO-C policy.
"""
...
def _git_info() -> Dict[str, Any]:
    """Collect git commit, dirty status, and evaluator file state."""
    info: Dict[str, Any] = {
        "commit": None,
        "dirty": None,
        "dirty_files": [],
        "evaluator_file_sha256": _file_sha256(__file__),
    }
    try:
    ...
    try:
        dirty_output = subprocess.check_output(
            ["git", "status", "--short"], cwd=Path(__file__).resolve().parents[3], text=True
        ).strip()
        info["dirty"] = bool(dirty_output)
        info["dirty_files"] = [line.strip() for line in dirty_output.splitlines() if line.strip()]
    except Exception:
        pass
    return info


def generate_long_horizon_requests(
    num_nodes: int,
    rng: np.random.RandomState,
    num_requests: int = 10000,
    arrival_interval: float = 0.0625,
    ...
    """Generate a long-horizon request sequence with optional Poisson/Exponential traffic.

    Each request samples a fresh source node uniformly from the topology.  The
    destination is not fixed here; in the compute-optical framework it is
    determined by the edge server selected by Agent-C.

    ``arrival_interval`` is interpreted as the mean inter-arrival time.  When
    ``poisson_arrivals`` is True, inter-arrival times are drawn from an
    exponential distribution with mean ``arrival_interval``.  Otherwise the
    original deterministic spacing is used (mean inter-arrival =
    ``arrival_interval``).

    When ``exponential_holding`` is True, holding times are exponential with
    mean ``(holding_min + holding_max) / 2``.  Otherwise they are uniform in
...
def _select_c_action(
    ...
    baseline_name = c_mode[:-2] if c_mode.endswith("_c") else c_mode
...
def _deep_rmsa_style_k50_action(obs_r: Dict[str, Any], mod_reg) -> Optional[int]:
    ...
    for action_idx in legal:
        ...
        # DeepRMSA-like score: reward tight fit, short path, low slot consumption,
        # and slight first-fit bias to encourage spectrum packing.
        score = (
            -1.00 * waste
...
def _run_episode(
    ...
    for step_idx, req in enumerate(requests):
        ...
        if step_idx < warmup_requests:
            # Warmup phase: advance the network/server state but do not record
            # metrics, so the measurement window starts from a steady state.
            continue
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    requested_c = {c for _, c, _ in methods}
    requested_r = {r for _, _, r in methods}

    # Warn when ppo_c is evaluated with the placeholder default checkpoint.
    # Heuristic C modes ignore agent_c, so the warning is only relevant for ppo_c.
    if "ppo_c" in requested_c:
        default_ckpt = "sa_hmarl/checkpoints/agent_c_delayaware_v2_snap24_best.pt"
        if args.agent_c_checkpoint == default_ckpt:
            import warnings
            warnings.warn(
                f"Using the default snap24 Agent-C checkpoint for ppo_c mode. "
                f"For imported topologies, override --agent_c_checkpoint with the "
                f"topology-appropriate r_feasibility_safe_last checkpoint.",
                UserWarning,
                stacklevel=2,
            )
    ...
    for seed in seeds:
        ...
        for _ in range(args.episodes):
            episodes.append(generate_long_horizon_requests(
    ...
    for method_name, c_mode, r_mode in methods:
        ...
        # Aggregate ranker profile stats across seeds.
        ...
        # Aggregate split distribution across seeds (useful for fixed-split ablations)
        if "split_dist" in sample:
            all_splits = set()
            for row in spec["per_seed"].values():
                all_splits.update(row["split_counts"].keys())
            total_counts = {split_id: sum(row["split_counts"].get(split_id, 0) for row in spec["per_seed"].values()) for split_id in all_splits}
            total_n = sum(total_counts.values())
            spec["aggregate"]["split_dist"] = {
                str(split_id): (count / total_n if total_n > 0 else 0.0)
                for split_id, count in sorted(total_counts.items())
            }

        # Aggregate modulation and server distributions across seeds.
        def _aggregate_distribution(key: str, count_key: str):
            if count_key not in sample:
                return
            all_keys = set()
            for row in spec["per_seed"].values():
                all_keys.update(row[count_key].keys())
            total_counts = {k: sum(row[count_key].get(k, 0) for row in spec["per_seed"].values()) for k in all_keys}
            total_n = sum(total_counts.values())
            spec["aggregate"][key] = {
                str(k): (count / total_n if total_n > 0 else 0.0)
                for k, count in sorted(total_counts.items())
            }

        _aggregate_distribution("mod_dist", "mod_counts")
        _aggregate_distribution("server_dist", "server_counts")
    ...
    return {
        ...
        "metadata": {
            ...
            "ranker_config": {
                ...
                "ensure_ksp": args.ranker_ensure_ksp,
                "ppo_top_k": args.ranker_ppo_top_k,
                "num_random_candidates": args.ranker_num_random_candidates,
                "min_candidates": args.ranker_min_candidates,
            },
        },
    }
...
def _write_markdown(path: Path, report: Dict[str, Any]) -> None:
    ...
    # Ranker profiling breakdown (only if at least one method collected profile data)
    if any("profile_stats" in spec.get("aggregate", {}) for spec in report["methods"].values()):
        lines.extend(["", "## Ranker Profile Breakdown (v1.3)", ""])
        lines.append("| Method | build_action_features | legal extract | candidate select | feature batch | normalize | ranker forward | total ranker |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
        for name, spec in report["methods"].items():
            prof = spec["aggregate"].get("profile_stats", {})
            if not prof:
                continue
            lines.append(
                f"| {name} | "
                f"{prof.get('r_feature_build_mean_ms', 0.0):.3f} | "
                f"{prof.get('legal_extract_mean_ms', 0.0):.3f} | "
                f"{prof.get('candidate_select_mean_ms', 0.0):.3f} | "
                f"{prof.get('feature_batch_mean_ms', 0.0):.3f} | "
                f"{prof.get('normalize_mean_ms', 0.0):.3f} | "
                f"{prof.get('ranker_forward_mean_ms', 0.0):.3f} | "
                f"{prof.get('total_ranker_policy_mean_ms', 0.0):.3f} |"
            )
    ...
    if meta:
        ...
        lines.append(f"- Evaluator SHA-256: `{git.get('evaluator_file_sha256', 'N/A')}`")
        ckpt = meta.get("checkpoint_sha256", {})
        lines.append(f"- Agent-C checkpoint SHA-256: `{ckpt.get('agent_c', 'N/A')}`")
        lines.append(f"- Agent-R checkpoint SHA-256: `{ckpt.get('agent_r', 'N/A')}`")
        lines.append(f"- Ranking checkpoint SHA-256: `{ckpt.get('ranking', 'N/A')}`")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--warmup_requests", type=int, default=2000,
                        help="Number of initial requests to discard from metrics (steady-state measurement).")
    ...
    parser.add_argument("--poisson_arrivals", action="store_true",
                        help="Use Poisson arrivals (exponential inter-arrival).")
    parser.add_argument("--exponential_holding", action="store_true",
                        help="Use exponential holding times with mean (holding_min+holding_max)/2.")
    ...
    parser.add_argument("--arrival_interval", type=float, default=0.0625,
                        help="Mean inter-arrival time (deterministic or exponential mean).")
