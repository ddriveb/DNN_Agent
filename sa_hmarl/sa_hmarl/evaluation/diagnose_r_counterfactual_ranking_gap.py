"""Gap diagnostic: why Counterfactual rank-only still trails DeepRMSA.

Runs three sets of analyses on the same request traces used for the formal
system comparison:

1. Separate trajectories for counterfactual_rank_only / ppo_r / deep_rmsa on
   identical traces, recording per-request legal R action count,
   selected_valid_r_actions, selected action features, and outcome
   (blocked / reason).
2. Aggregate action agreement, failure-mode overlap, blocking by legal count,
   blocking by selected_valid, and selected-action feature distributions.
3. Output a JSON + markdown gap diagnostic report.
"""
...
def evaluate(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    rank_pack = (rank_model, rank_mean, rank_std)
    ...
    methods = ["rank_only", "ppo_r", "deep_rmsa"]
    all_records = []
    separate_metrics = {}
    for method in methods:
        records, metrics = _run_separate_trajectory(
            method, episodes_by_seed, agent_c, agent_r, deep_rmsa, rank_pack, args
        )
    ...
    agreement_rank_deep, n_rank_deep = _action_agreement(all_records, "rank_only", "deep_rmsa")
    agreement_rank_ppo, n_rank_ppo = _action_agreement(all_records, "rank_only", "ppo_r")
    agreement_deep_ppo, n_deep_ppo = _action_agreement(all_records, "deep_rmsa", "ppo_r")

    # Failure-mode overlap.
    rank_records = { (r["seed"], r["req_id"]): r for r in all_records if r["method"] == "rank_only" }
    deep_records = { (r["seed"], r["req_id"]): r for r in all_records if r["method"] == "deep_rmsa" }
    ppo_records = { (r["seed"], r["req_id"]): r for r in all_records if r["method"] == "ppo_r" }
    failure_overlap = []
    for key, rr in rank_records.items():
        dr = deep_records.get(key)
        pr = ppo_records.get(key)
        if dr is None or pr is None:
            continue
        failure_overlap.append({
            "rank_blocked": bool(rr["blocked"]),
            "deep_blocked": bool(dr["blocked"]),
            "ppo_blocked": bool(pr["blocked"]),
            "rank_only_fail_deep_success": bool(rr["blocked"] and not dr["blocked"]),
            "deep_fail_rank_success": bool(dr["blocked"] and not rr["blocked"]),
            "both_fail": bool(rr["blocked"] and dr["blocked"]),
            "both_success": bool(not rr["blocked"] and not dr["blocked"]),
        })
    reason_counts = {}
    for method in methods:
        reasons = [r["reason"] for r in all_records if r["method"] == method and r["blocked"]]
        reason_counts[method] = {
            "total": len(reasons),
            "no_suitable_block": int(sum(1 for x in reasons if x == "no_suitable_block")),
            "server_overload": int(sum(1 for x in reasons if x == "server_overload")),
            "other": int(sum(1 for x in reasons if x and x not in ("no_suitable_block", "server_overload"))),
        }
    ...
    report = {
        ...
        "agreement": {
            "rank_only_vs_deep_rmsa": {"rate": agreement_rank_deep, "pairs": n_rank_deep},
            "rank_only_vs_ppo_r": {"rate": agreement_rank_ppo, "pairs": n_rank_ppo},
        ...
        "failure_overlap": {
            "samples": len(failure_overlap),
            "rank_only_fail_deep_success_rate": float(np.mean([x["rank_only_fail_deep_success"] for x in failure_overlap])) if failure_overlap else 0.0,
            "deep_fail_rank_success_rate": float(np.mean([x["deep_fail_rank_success"] for x in failure_overlap])) if failure_overlap else 0.0,
            "both_fail_rate": float(np.mean([x["both_fail"] for x in failure_overlap])) if failure_overlap else 0.0,
            "both_success_rate": float(np.mean([x["both_success"] for x in failure_overlap])) if failure_overlap else 0.0,
        },
...
def _write_markdown(path: Path, report: Dict[str, Any]) -> None:
    lines = ["# Counterfactual Rank-Only vs DeepRMSA Gap Diagnostic", ""]
    ...
    lines += ["## 2. Per-seed blocking", ""]
    seed_keys = sorted(next(iter(report["separate_metrics"].values()))["per_seed"].keys(), key=int)
    lines += ["| Method | " + " | ".join(seed_keys) + " |", "|---|---|---|---|---|---|"]
    for method, spec in report["separate_metrics"].items():
        per_seed = spec["per_seed"]
        vals = " | ".join(f"{per_seed[seed]['blocking_rate']:.2%}" for seed in seed_keys)
        lines.append(f"| {method} | {vals} |")
    lines.append("")

    lines += ["## 3. R-action agreement (same state)", ""]
    ...
    lines += ["## 4. Failure-mode overlap (rank-only vs DeepRMSA)", ""]
    fo = report["failure_overlap"]
    lines += [
        f"- Common samples: {fo['samples']}",
        f"- rank-only blocked, DeepRMSA success: {fo['rank_only_fail_deep_success_rate']:.2%}",
        f"- DeepRMSA blocked, rank-only success: {fo['deep_fail_rank_success_rate']:.2%}",
        f"- Both blocked: {fo['both_fail_rate']:.2%}",
        f"- Both success: {fo['both_success_rate']:.2%}",
        "",
    ]

    lines += ["## 5. Block reason counts", ""]
    lines += ["| Method | Total blocked | NSB | Overload | Other |", "|---|---:|---:|---:|---:|"]
    for method, counts in report["reason_counts"].items():
        lines.append(
            f"| {method} | {counts['total']} | {counts['no_suitable_block']} | "
            f"{counts['server_overload']} | {counts['other']} |"
        )
    lines.append("")

    lines += ["## 6. Blocking by legal R action count", ""]
    ...
    lines += ["## 7. Blocking by selected_valid_r_actions", ""]
    lines += ["| Method | Bin | Count | Blocked | Blocking rate |", "|---|---|---|---:|---:|"]
    ...
    lines += ["## 8. Selected action feature summary", ""]
    ...
    lines += ["## 9. Interpretation and targeted v2 improvements", ""]
    lines += [
        "1. If rank-only fails where DeepRMSA succeeds at high rate, candidate coverage is likely insufficient;",
        "   v2 should expand the candidate set.",
        "2. If action agreement is low and feature means differ (e.g., rank-only uses lower modulation or larger waste),",
        "   the return coefficients may be mis-weighted; v2 should emphasize blocking/NSB more.",
        "3. If rank-only succeeds but delay/overload worsens, conservative rerank/margin should be used.",
        "4. Next steps for v2:",
        "   - Increase PPO top-K and diversity heuristics to enlarge candidate coverage.",
        "   - Raise future_block / future_nsb coefficients and reduce delay/fs coefficients.",
        "   - Try λ/margin sweeps so rank-only overrides PPO-R only when confident.",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output_json", default="sa_hmarl/experiments/r_counterfactual_ranking_gap_diagnostic.json")
    parser.add_argument("--output_md", default="sa_hmarl/experiments/r_counterfactual_ranking_gap_diagnostic.md")
    return parser
