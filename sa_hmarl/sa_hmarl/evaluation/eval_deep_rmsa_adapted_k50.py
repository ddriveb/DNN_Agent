#!/usr/bin/env python3
"""Evaluate a trained adapted DeepRMSA K=50 checkpoint on fixed-C / all-OD seeds."""
...
def _load_agent(ckpt_path: str, env, device: str = "cpu") -> DeepRMSAAdaptedK50Agent:
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    cfg = ckpt.get("protocol_config", {})
    num_nodes = ckpt.get("num_nodes", env.net.NUM_NODES)
    num_slots = ckpt.get("num_slots", env.net.num_slots)

    # Fail closed on topology / slot mismatch.
    if num_nodes != env.net.NUM_NODES:
        raise ValueError(f"Checkpoint topology mismatch: ckpt nodes={num_nodes}, env nodes={env.net.NUM_NODES}")
    if num_slots != env.net.num_slots:
        raise ValueError(f"Checkpoint slot mismatch: ckpt slots={num_slots}, env slots={env.net.num_slots}")

    k_path = ckpt.get("k_path", 50)
    num_modulations = ckpt.get("num_modulations", 4)
    m_blocks = ckpt.get("m_blocks", 10)
    if k_path * num_modulations * m_blocks != 2000:
        raise ValueError(f"Checkpoint action space mismatch: {k_path}*{num_modulations}*{m_blocks} != 2000")

    mod_reg = ModulationRegistry.from_profile(cfg.get("modulation_profile", "default"))
...
def _run_eval(env, requests, agent, fixed_split_id, seed):
    summary = MethodSummary(method="Topology-matched adapted DeepRMSA K=50 hops")
    ...
    for step_idx, req in enumerate(requests):
        ...
        # Verify legality (should always be true due to mask).
...
def main():
    ...
    agent = _load_agent(str(ckpt_path), env_for_nodes, args.device)
    print(f"[eval] Loaded checkpoint {ckpt_path}")
    print(f"[eval] Parameters: {agent.count_parameters()}")
    ...
    for seed in seeds:
        ...
        env.reset(requests)
        summary, records = _run_eval(env, requests, agent, args.fixed_split_id, seed)
        per_seed.append({"seed": seed, "summary": summary.to_dict()})
        all_records.extend(records)
        print(f"[eval][seed={seed}] blocking={summary.blocking_rate():.4%}")

    total_summary = MethodSummary(method="Topology-matched adapted DeepRMSA K=50 hops")
    ...
    print(f"[eval] Overall blocking={total_summary.blocking_rate():.4%}")
    print(f"[eval] Wrote {out_dir / 'eval.json'}")
    print(f"[eval] Done in {time.time() - t_start:.1f}s")
