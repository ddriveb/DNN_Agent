"""Cross-topology generalization analysis for SA-HMARL.

Train on net1, evaluate on net2/net3/nsfnet.

Usage (from project root):
    PYTHONPATH=sa_hmarl .venv/bin/python sa_hmarl/scripts/run_cross_topology.py
"""
import sys
...
from sa_hmarl.evaluation.eval_joint import (
    _run_single_seed, _aggregate_across_seeds, METHODS,
)
from sa_hmarl.utils.checkpoint import load_checkpoint


def run_topology(topology, seeds, episodes, requests_per_episode, args_common):
    """Run evaluation for one topology across multiple seeds.
    Agent checkpoints are loaded from net1 training (fixed)."""
    env = make_env(
        topology, args_common.num_slots, args_common.num_servers,
        seeds[0], slot_bw_hz=args_common.slot_bw_hz,
        guard_band_fs=args_common.guard_band_fs,
...
def main():
    ...
    parser.add_argument("--train_topology", type=str, default="net1")
    parser.add_argument("--eval_topologies", type=str, default="net2,net3,nsfnet")
    ...
    eval_topologies = [t.strip() for t in args.eval_topologies.split(",")]
    target = "Agent-C-DQN + Agent-R-DQN"
    ksp_baseline = "Agent-C-DQN + KSP-BF"

    print("=" * 120)
    print(f"Cross-Topology Generalization: Train on {args.train_topology}, Eval on {eval_topologies}")
    print(f"Seeds: {seeds} | Episodes/seed: {args.episodes} | Requests/episode: {args.requests_per_episode}")
    print("=" * 120)

    results = []
    for topo in eval_topologies:
        print(f"\n[Running topology={topo}]", file=sys.stderr)
        overall, aggregated, win_count = run_topology(topo, seeds, args.episodes, args.requests_per_episode, args)
        results.append({"topology": topo, "overall": overall, "aggregated": aggregated, "win_count": win_count})

    # Print table
    header = (f"{'Topology':>10} {'Blocking':>12} {'Success':>12} {'Reward':>12} "
    ...
    for row in results:
        ...
        print(f"{topo:>10} {o['blocking_rate'][0]:>12.3f} {o['success_rate'][0]:>12.3f} "
    ...
    # Baseline comparison per topology
    print("\nBlocking: Joint vs Agent-C + KSP-BF per topology")
    print("-" * 120)
    print(f"{'Topology':>10} {'Joint':>12} {'KSP-BF':>12} {'Delta':>10} {'Delta%':>10}")
    print("-" * 120)
    for row in results:
        topo = row["topology"]
        ...
        print(f"{topo:>10} {joint_blk:>12.3f} {ksp_blk:>12.3f} {delta:>10.3f} {delta_pct:>9.1f}%")
    print("=" * 120)
