#!/usr/bin/env bash
# Run df_fixed0 full comparison for one imported topology.
# Usage: bash run_df_fixed0_topology.sh <topology> <arrival_interval> <edge_cost_max> <output_basename>
set -euo pipefail
...
PYTHONPATH=sa_hmarl .venv/bin/python -m sa_hmarl.evaluation.eval_long_horizon_system_comparison \
  ...
  --methods "df_fixed0_c+ksp_ff_k50_hops,df_fixed0_c+v12_k50_hops" \
  ...
  --output_json "sa_hmarl/experiments/heuristic_c_v13_vs_kspff/${OUT_BASE}_df_fixed0_full.json" \
  --output_md "sa_hmarl/experiments/heuristic_c_v13_vs_kspff/${OUT_BASE}_df_fixed0_full.md"
