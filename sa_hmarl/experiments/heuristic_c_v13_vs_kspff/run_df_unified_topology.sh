#!/usr/bin/env bash
# Run df_c unified main-table comparison for one imported topology.
# Usage: bash run_df_unified_topology.sh <topology> <arrival_interval> <edge_cost_max> <output_basename>
set -euo pipefail
...
PYTHONPATH=sa_hmarl .venv/bin/python -m sa_hmarl.evaluation.eval_long_horizon_system_comparison \
  ...
  --output_json "sa_hmarl/experiments/heuristic_c_v13_vs_kspff/${OUT_BASE}_df_unified_full.json" \
  --output_md "sa_hmarl/experiments/heuristic_c_v13_vs_kspff/${OUT_BASE}_df_unified_full.md"
