"""
Predictor MVP — NumPy-only fallback (no PyTorch required).
Runs the same ablation without external DL dependencies.
"""
import numpy as np
from env import OpticalNetwork
from mapper import KSPMapper
from encoder import Encoder
from numpy_predictor import NumpyPredictor
from dataset import DatasetGenerator


def run_experiment(encoder_version="v2", num_samples=20000, epochs=30):
    print(f"\n{'='*60}")
    print(f"Encoder version: {encoder_version}  (NumPy Predictor)")
    print(f"{'='*60}")
    ...
    predictor = NumpyPredictor(state_dim, num_partitions=3, num_servers=14, hidden_dim=64, seed=42)
    history = predictor.train(samples, epochs=epochs, lr=1e-3, batch_size=256)
    ...
    z = np.array([encoder.encode(0, 5)], dtype=np.float32)
    X = predictor._build_X(np.array([0]), np.array([0]), np.array([5]), z)
    t0 = time.time()
    for _ in range(500):
        _ = predictor.predict_proba(X)
...
if __name__ == "__main__":
    ...
    for version in ["v0", "v1", "v2"]:
        results[version] = run_experiment(version, num_samples=20000, epochs=30)
