"""
Quick validation: noise augmentation at 10K sample size.
Tests sigma=0.15 vs no noise on 10K Random100 fine-tune samples.
"""
import numpy as np
import torch
import time
...
if __name__ == "__main__":
    log("Pre-generating 10K data (this will take ~30 min)...")
    t0 = time.time()
    ft_10k = generate_samples("random100", "v2b", 10000, seed=102)
    ...
    log(f"  FT_10K={len(ft_10k)} SR={np.mean([s['success'] for s in ft_10k]):.3f}")
    log(f"  Test={len(test_samples)} SR={np.mean([s['success'] for s in test_samples]):.3f}")

    configs = [
        (0.00, ft_10k, "no_noise_10k"),
        (0.15, ft_10k, "noise_0.15_10k"),
    ]
    ...
    # Summary
    log("\n" + "="*75)
    log("NOISE + 10K SAMPLE VALIDATION (Random100, 5 epochs)")
    log("="*75)
