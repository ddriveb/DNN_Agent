"""
Selector+Mapper closed-loop demo v3.
Focus: high load, proper metrics, frag-aware strategy.
"""
import os
import sys
import numpy as np
...
def calibrate_load(topology, num_slots):
    """Return (arrival_interval, holding_time) that creates ~15% Shortest blocking."""
    if topology == "nsfnet":
        # 14 nodes, 22 links, 32 slots = 704 total
        # Need ~20 concurrent requests to stress
        return 0.25, 4.0   # ~16 concurrent, should yield 10-20% BR
    elif topology.startswith("random"):
        # 100 nodes, ~300 links, 128 slots = 38400 total
        # Need ~400 concurrent to stress
        return 0.05, 10.0  # ~200 concurrent, moderate stress
    return 0.5, 5.0

# --- Helpers -----------------------------------------------------------------
def generate_train_data(topology, num_slots, n_samples, seed=42):
    print(f"  Generating {n_samples} training samples ({topology}) ...")
    arr, ht = calibrate_load(topology, num_slots)
    # Use SAME load as test to get realistic success/fail mix
    # (slightly higher to ensure enough negative samples)
    arr_train = arr * 0.8
    # Scale preload with network size
    ...
    return gen.generate(n_samples, arrival_rate=arr_train, avg_holding_time=ht,
...
def strategy_predictor_fragaware(net, mapper, selector, src, dst, bw):
    """
    Frag-aware: among paths with prob > threshold, pick the one with
    lowest estimated frag_change. Fallback to max_prob if none qualify.
    """
    best, candidates = selector.select(src, dst, bw)
    
    # Get all candidate probs
    probs = [c["success_prob"] for c in candidates]
    max_prob = max(probs) if probs else 0
    threshold = max_prob * 0.9  # within 10% of best
    
    # Filter high-prob candidates and estimate frag impact
    qualified = [c for c in candidates if c["success_prob"] >= threshold]
    if len(qualified) <= 1:
        return mapper.execute_path(src, dst, bw, path_id=best["path_id"])
    
    # Evaluate frag impact by actually trying each qualified path
    best_frag = None
    best_path_id = best["path_id"]
    for cand in qualified:
        pid = cand["path_id"]
        # Try without actually allocating
...
def experiment(topology, num_slots, num_requests, bw_choices, label):
    ...
    print(f"  Load: arr={arr:.2f}, ht={ht:.1f}  →  stress level ~{ht/arr:.0f}x")
    print(f"  Train: {TRAIN_SAMPLES} samples, {TRAIN_EPOCHS} epochs")
    print(f"{'='*70}")

    # Train
    train_samples = generate_train_data(topology, num_slots, TRAIN_SAMPLES, seed=SEED)
...
def main():
    print("=" * 70)
    print("SELECTOR+MAPPER CLOSED-LOOP DEMO v3")
