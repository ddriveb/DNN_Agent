"""Training loop for Hybrid Predictor (v2b + GAT)."""
import numpy as np
import torch
...
class HybridDataset(Dataset):
    """Dataset for Hybrid predictor (needs both z and link-as-node features)."""
...
def hybrid_collate_fn(batch):
    """Custom collate for Hybrid: stack tensors, keep converted_edge_index as scalar."""
    keys = batch[0].keys()
    result = {}
    for key in keys:
...
def train_hybrid_predictor(predictor, train_loader, val_loader, epochs=30, lr=1e-3,
                             class_weight=True, early_stop_patience=3):
    optimizer = optim.Adam(predictor.parameters(), lr=lr)
    mse = nn.MSELoss()
    history = {"train_loss": [], "val_acc": [], "val_auc": [], "val_mae": [], "val_ece": []}

    # Compute class weights from first batch (approximation)
    pos_weight = 1.0
    if class_weight:
        all_labels = []
        ...
        if n_neg > 0 and n_pos > 0:
            pos_weight = n_pos / n_neg  # up-weight negatives
            print(f"    Class weight: pos={n_pos}, neg={n_neg}, weight={pos_weight:.2f}")
    ...
    for epoch in range(epochs):
        ...
        metrics = evaluate_hybrid_predictor(predictor, val_loader)
        ...
        if metrics["auc"] > best_auc:
            ...
            patience_counter += 1
            if patience_counter >= early_stop_patience and epoch >= 4:
...
def evaluate_hybrid_predictor(predictor, loader):
    predictor.eval()
