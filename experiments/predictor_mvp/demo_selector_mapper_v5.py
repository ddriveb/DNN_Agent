"""
Selector+Mapper closed-loop demo v5.
- FIXED: Uses corrected DatasetGenerator (per-path-id labels)
- NEW: Configurable K (num_paths) for larger candidate space
"""
import os
import numpy as np
import torch
...
TRAIN_SAMPLES = int(os.environ.get("TRAIN_SAMPLES", "1000"))
...
def generate_train_data_v5(topology, num_slots, n_samples, k, seed=42):
    """Use FIXED DatasetGenerator with correct per-path-id labels."""
    print(f"  Generating {n_samples} training samples ({topology}, K={k}) ...")
    ...
    mapper = KSPMapper(net, k=k)
    encoder = HybridEncoder(net, k=k)
    gen = DatasetGenerator(net, mapper, encoder, num_partitions=k, seed=seed)
    samples = gen.generate(n_samples, arrival_rate=arr, avg_holding_time=ht,
...
def train_hybrid_model(samples, k, epochs=5):
    print(f"  Training Hybrid Predictor ({epochs} epochs, K={k}) ...")
    np.random.seed(SEED)
    ...
    model = HybridPredictor(
        ...
        num_paths=k, max_servers=MAX_SERVERS, hidden_dim=64,
    )
...
def strategy_random(net, mapper, selector, src, dst, bw):
    k = len(mapper._path_cache.get((src, dst), []))
    if k == 0:
        return {"success": False, "path": None, "start_slot": None, "delay": None, "frag_change": 0.0}
    path_id = np.random.randint(0, k)
...
def experiment(topology, num_slots, num_requests, bw_choices, k, label):
    arr, ht = calibrate_load(topology, num_slots)
    print(f"\n{'='*70}")
    print(f"EXPERIMENT: {label}")
    print(f"  Topology: {topology}, Slots: {num_slots}, K={k}")
    print(f"  Load: arr={arr:.2f}, ht={ht:.1f}")
    print(f"  Train: {TRAIN_SAMPLES} samples, {TRAIN_EPOCHS} epochs")
    print(f"{'='*70}")

    train_samples = generate_train_data_v5(topology, num_slots, TRAIN_SAMPLES, k, seed=SEED)
    hybrid_model = train_hybrid_model(train_samples, k, epochs=TRAIN_EPOCHS)
...
def main():
    print("=" * 70)
    print("SELECTOR+MAPPER CLOSED-LOOP DEMO v5")
    print(f"  K={NUM_PATHS}, Train={TRAIN_SAMPLES}, Epochs={TRAIN_EPOCHS}")
    print("  FIXED DatasetGenerator + Configurable K")
    print("=" * 70)

    results = []
    # NSFNET with different K values
    for k in [3, 5, 10]:
        results.append(experiment(
            topology="nsfnet", num_slots=32, num_requests=2000,
            bw_choices=[1, 2, 4, 8], k=k,
            label=f"NSFNET-K{k}"
        ))
    ...
    print(f"{'='*70}")
    print(f"{'Experiment':<15} {'K':>3} {'Base BR':>10} {'Pred-MP':>10} {'Pred-Frag':>10} {'Improve':>10}")
    print("-" * 70)
    for r in results:
        print(f"{r['label']:<15} {r['k']:>3} {r['shortest_br']*100:>9.2f}% {r['pred_mp_br']*100:>9.2f}% "
